...
def format_digest(date: str, summaries: list[dict]) -> str:
    ...
    # Group by cluster, preserving the order clusters appear in summaries
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("input", help="JSON from download step (or - for stdin)")
    ...
    if context:
        print(f"{_P} Loaded research context ({len(context)} chars)", file=sys.stderr)

    print(f"{_P} Processing {len(papers)} papers...", file=sys.stderr)
    ...
    print(f"{_P} Digest → {digest_path}", file=sys.stderr)
